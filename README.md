...
```bash
# 1. Bedrock 환경변수 설정
bash scripts/01-setup-bedrock-env.sh
source ~/.bashrc

# 2. VS Code 확장 설정 (code-server 사용 시)
bash scripts/02-setup-vscode-settings.sh

# 3. 플러그인 + MCP 서버 설치
bash scripts/03-setup-plugins-and-mcp.sh

# 4. Claude Code 업데이트 (선택)
bash scripts/04-update-claude.sh

# 5. 커스텀 플러그인 설치 (선택)
bash scripts/05-setup-custom-plugin.sh
```
